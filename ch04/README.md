...
GraphQL fragments help keep documents shorter and easier to understand (p. 59)
...
GraphQL documents are processed at runtime using a schema module that’s already been checked for common errors and optimized for better performance (p. 59)
...
A type module's purpose is to contain a set of types for inclusion in a schema (p. 60)
...
The import tools, `import_types` and `import_fields`, don’t establish any structural constraints for the way that you arrange your Absinthe-related modules (p. 63)
...
Modeling all these search results... as search results (pp. 64-65)
```elixir
...
Group of `:menu_item` records with a name and description (p. 65)
```elixir
object :category do
  field :name, :string
...
The `union` macro is used to create our type, and works a lot like object. The `types` macro, used inside the union scope, sets its types (p. 66)

Abstract types like unions and interfaces need a way to determine the concrete type for a value (p. 66)
...
The search resolver function executes a database query, checking a pattern against names and descriptions for each table, and returns the combined results (p. 67)

Fragments are a way to write chunks of GraphQL that can target a specific type; you can see where we’re defining and inserting fragments on lines 3 and line 6 (p. 68)
...
The `__typename` introspection field always returns the concrete GraphQL type name that’s in the associated scope (p. 70)
...
Use the `interface` macro instead of the `union` macro, remove the `types` macro usage, use the field macro (p. 72)
...
If we wanted to retrieve information about menu items that belonged to any categories that were returned from a search, we’d still need to have a wrapping fragment type, such as getting a category's `:items` because it isn’t declared on the interface. It’s not a field that’s shared with other object types (p. 74)
```graphql
query Search($term: String!) {
  search(matching: $term) {
    name
...
Named fragment are defined outside the GraphQL operation using the fragment keyword (p. 74)
...
Fields specified multiple times are de-duped, so they are safe to be repeated, which is especially helpful when using fragments.
